    ...
    var fusioncharts2 = new FusionCharts({
        ...
        dataSource: {
            "chart": {
                ...
                "showRealTimeValue": "0",
                "theme": "fusion",
                "yAxisMaxValue": (ethereumDataHandler().toString() + 20),
                "yAxisMinValue": (ethereumDataHandler().toString() - 20),
            },
            "categories": [{
                "category": [{
                    "label": clientDateTime().toString()
            ...
            "dataset": [{
                "data": [{
                    "value": ethereumDataHandler().toString()
                }]
            }]
        },
        "events": {
            "initialized": function (e) {
                function updateData() {
                    // Get reference to the chart using its ID
                    var chartRef = FusionCharts("stockRealTimeChart2"),
                        x_axis = clientDateTime(),
                        y_axis = ethereumDataHandler(),
                        strData = "&label=" + x_axis + "&value=" + y_axis;
                    // Feed it to chart.
                    chartRef.feedData(strData);
                }
    ...
    var fusioncharts3 = new FusionCharts({
        ...
        dataSource: {
            "chart": {
                ...
                "showRealTimeValue": "0",
                "theme": "fusion",
                "yAxisMaxValue": (litecoinDataHandler().toString() + 20),
                "yAxisMinValue": (litecoinDataHandler().toString() - 20),
            },
            "categories": [{
                "category": [{
                    "label": clientDateTime().toString()
            ...
            "dataset": [{
                "data": [{
                    "value": litecoinDataHandler().toString()
                }]
            }]
        },
        "events": {
            "initialized": function (e) {
                function updateData() {
                    // Get reference to the chart using its ID
                    var chartRef = FusionCharts("stockRealTimeChart3"),
                        x_axis = clientDateTime(),
                        y_axis = litecoinDataHandler(),
                        strData = "&label=" + x_axis + "&value=" + y_axis;
                    // Feed it to chart.
                    chartRef.feedData(strData);
                }
